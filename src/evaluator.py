# evaluator.py — Evaluate the Trained Model
# ============================================================
...
# ──────────────────────────────────────────────────────────────
# get_predictions  —  run model on a loader
# ──────────────────────────────────────────────────────────────
...
def predict(model, loader, cfg):
    """
    Run model on test loader and return predictions.
    Call this ONCE per setting, then pass results to
    show_accuracy, show_metrics and show_confusion.
    This avoids running the model 3 times unnecessarily.
    """
    device = cfg["DEVICE"]
...
def show_confusion(y_pred, y_true, class_names):
    ...
    for x,y,label,value,color in boxes:
        ...
        ax.text(x+0.5,y+0.60,label,
                ha="center",
                va="center",
                fontsize=13,
                fontweight="bold")

        ax.text(x+0.5,y+0.30,f"{value:,}",
                ha="center",
                va="center",
                fontsize=15)
